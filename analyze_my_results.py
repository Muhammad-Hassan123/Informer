...
""")
    ...
    checkpoint_dirs = [
        "checkpoints/informer_LINK_ULTRA_ftMS_sl90_ll45_pl10_ensemble_model_1",
        "checkpoints/informer_LINK_ULTRA_ftMS_sl90_ll45_pl10_ensemble_model_2", 
        "checkpoints/informer_LINK_ULTRA_ftMS_sl90_ll45_pl10_ensemble_model_3"
    ]
    
    for i, checkpoint_dir in enumerate(checkpoint_dirs, 1):
        if os.path.exists(checkpoint_dir):
            files = os.listdir(checkpoint_dir)
            print(f"✅ Model {i} checkpoint: {len(files)} files saved")
            if 'checkpoint.pth' in files:
                print(f"   📁 Model weights: checkpoint.pth ✅")
            if 'pred.npy' in files:
                print(f"   📊 Predictions: pred.npy ✅")
            else:
                print(f"   ❌ Predictions: pred.npy missing (prediction step failed)")
        else:
            print(f"❌ Model {i} checkpoint: Directory not found")
